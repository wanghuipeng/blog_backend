...
const url = `https://segmentfault.com/channel/backend`;;
(async() => {
        ...
        if (items.length >= 1) {
            items.each((index, item) => {
                ...
                if (it.find('.news-img')) {
                    ...
                    sd_thumbnail = `http://127.0.0.1:3000/downloads/backEnd/${index}.jpg`
                } else {
                    sd_thumbnail = ''
                }
                // 被赞次数
    ...
    imgs.map((item, index) => {
        try {
            ...
            let path = `../public/downloads/backEnd/${index}.jpg`;
    ...
    // 存储到数据库
    result.map(item => {
        const addData = new BackEndModel(item)
        addData.save()
    })
